...
await dotenv.config(); //Carga las variables de entorno del archivo .env

const app = express(); //Instancia la aplicacion en una constante con nombre"App"
const PORT = process.env.SV_PORT || 3001; // Declara PORT como el valor dentro del archivo .env, si no llegase a estar disponible, se asigna el puerto 3001
...
app.get("/", (req, res) => {
  //primer endpoint de prueba, loguea el metodo http, de donde viene y responde texto plano
...
app.listen(PORT, () => {
  //loguea el puerto de escucha recien se arranca el server
  console.log(`Backend funcionando y escuchando en: http://localhost:${PORT}`);
});
